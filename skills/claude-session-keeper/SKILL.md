...
| `claude-keep pause <session…>` | **Archive** session(s): kill the tmux (if live) but **keep the registry row** (paused=1). `restore`/`tidy` skip a paused session, so it stays down until you resume — its transcript is untouched. Names as for `send`: chat title / bare tail / full tmux name (ambiguous → error). |
| `claude-keep resume <session…>` | Un-pause (paused=0). The next `restore` (run it, or wait for the timer) brings the session back **right where it left off** (`--resume`). |
